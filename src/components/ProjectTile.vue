<template>
    ...
    <div class="hidden lg:block lg:relative w-[25%] mx-auto my-[25px] h-[300px] rounded-lg overflow-hidden group">
    <!-- Background Image -->
    <div
      class="absolute inset-0 bg-cover bg-center transition-all duration-500 group-hover:opacity-100 m-4"
      :style="{ backgroundImage: `url(${imageURL})` }">
    </div>

    <!-- Overlay (Darkened Background) -->
    <div
      class="absolute inset-0 bg-black bg-opacity-60 transition-all duration-500 group-hover:bg-opacity-30">
    </div>

    <!-- Content -->
    <div class="absolute bottom-5 left-5 right-5 text-white transition-all duration-500 group-hover:bottom-10 m-2">
      <!-- Title -->
      <h2 class="text-xl font-semibold group-hover:text-accent transition-all duration-300">
        {{ title }}
      </h2>
      ...
      <!-- GitHub Link -->
      <a :href="gitURL" target="_blank" class="inline-block mt-3">
        <button
          class="bg-slate-700 hover:bg-slate-950 text-white font-mono font-bold py-2 px-4 rounded-full flex items-center">
          View on Github
          <svg class="ml-2" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 50 50"
            style="fill:#FFFFFF;">
            <path
